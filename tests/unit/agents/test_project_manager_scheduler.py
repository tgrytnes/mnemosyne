...
class TestSchedulerSetup:
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_scheduler_creates_background_scheduler(self, mock_scheduler_class):
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_scheduler_registers_pm_check_cycle_job(
        self, mock_scheduler_class, project_manager
    ):
        ...
        pm_check_job = None
        for call_args in add_job_calls:
            if call_args[1].get("id") == "pm_check_cycle":
                pm_check_job = call_args
                break

        assert pm_check_job is not None
        assert pm_check_job[1]["trigger"] == "interval"
        assert pm_check_job[1]["minutes"] == 30

    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_scheduler_registers_pressure_score_job(
        self, mock_scheduler_class, project_manager
    ):
        ...
        pressure_job = None
        for call_args in add_job_calls:
            if call_args[1].get("id") == "pressure_score_updater":
                pressure_job = call_args
                break

        assert pressure_job is not None
        assert pressure_job[1]["trigger"] == "interval"
        assert pressure_job[1]["hours"] == 1
...
class TestSchedulerLifecycle:
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_start_starts_scheduler(self, mock_scheduler_class, project_manager):
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_stop_shuts_down_scheduler(self, mock_scheduler_class, project_manager):
...
class TestJobExecution:
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_pm_check_cycle_job_calls_agent_method(
        self, mock_scheduler_class, project_manager
    ):
        ...
        for call_args in add_job_calls:
            if call_args[1].get("id") == "pm_check_cycle":
                pm_check_func = call_args[0][0]  # First positional arg is the function
                break
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_pressure_score_job_calls_update_method(
        self, mock_scheduler_class, project_manager
    ):
...
class TestErrorHandling:
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_pm_check_cycle_logs_errors_and_continues(
        self, mock_scheduler_class, project_manager
    ):
...
class TestConfigurableIntervals:
    ...
    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_custom_pm_check_interval(self, mock_scheduler_class, project_manager):
        ...
        assert pm_check_job[1]["minutes"] == 15

    @patch("mnemosyne.aletheia.agents.project_manager_scheduler.BackgroundScheduler")
    def test_custom_pressure_update_interval(
        self, mock_scheduler_class, project_manager
    ):
