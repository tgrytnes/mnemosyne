...
class TestSQLToObsidianSync:
    ...
    def test_sync_new_project_creates_file(self, mock_db_conn, mock_obsidian_vault, sample_project_from_sql):
    ...
    def test_sync_updates_sql_obsidian_path(self, mock_db_conn, mock_obsidian_vault, sample_project_from_sql):
    ...
    def test_sync_preserves_custom_sections(self, mock_db_conn, mock_obsidian_vault, sample_project_from_sql):
...
"""
    ...
    def test_sync_batch_projects(self, mock_db_conn, mock_obsidian_vault):
        ...
        projects = [
            {'id': 1, 'title': 'Project One', 'description': 'Desc 1', 'status': 'active',
             'discovered_by': 'test', 'discovery_id': 'd1', 'cluster_ids': ['c1'],
             'confidence_score': 0.8, 'created_at': datetime.now(timezone.utc),
             'updated_at': datetime.now(timezone.utc)},
            {'id': 2, 'title': 'Project Two', 'description': 'Desc 2', 'status': 'active',
             'discovered_by': 'test', 'discovery_id': 'd2', 'cluster_ids': ['c2'],
    ...
    def test_sync_skips_recently_synced(self, mock_db_conn, mock_obsidian_vault, sample_project_from_sql):
        """Test that sync skips projects synced within cooldown period"""
        from mnemosyne.aletheia.obsidian_sync.obsidian_sync import ObsidianSyncManager

        # Mark project as synced 30 seconds ago
        sample_project_from_sql['last_synced_to_obsidian'] = datetime.now(timezone.utc) - timedelta(seconds=30)

        sync_manager = ObsidianSyncManager(mock_db_conn, str(mock_obsidian_vault), sync_cooldown_seconds=60)
    ...
    def test_sync_force_bypasses_cooldown(self, mock_db_conn, mock_obsidian_vault, sample_project_from_sql):
        """Test that force=True bypasses cooldown"""
        from mnemosyne.aletheia.obsidian_sync.obsidian_sync import ObsidianSyncManager

        # Mark as recently synced
        sample_project_from_sql['last_synced_to_obsidian'] = datetime.now(timezone.utc) - timedelta(seconds=10)
...
class TestConflictDetection:
    ...
    def test_detect_conflict_when_both_modified(self, mock_db_conn, mock_obsidian_vault):
        ...
        cursor_mock.fetchone.return_value = (
            42, 'Test', sql_updated_at, last_synced_from_obsidian
        )
    ...
    def test_conflict_resolution_sql_wins(self, mock_db_conn, mock_obsidian_vault):
        ...
        with patch.object(sync_manager, 'detect_conflict', return_value=conflict):
            with patch.object(sync_manager, 'sync_project_to_obsidian') as mock_sync:
                ...
                mock_sync.assert_called_once_with(pytest.any_dict(), force=True)
    ...
    def test_conflict_resolution_obsidian_wins(self, mock_db_conn, mock_obsidian_vault):
        ...
        sync_manager = ObsidianSyncManager(
            mock_db_conn,
            str(mock_obsidian_vault),
            conflict_strategy='obsidian_wins'
        )
