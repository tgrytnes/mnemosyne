...
class EmailIngestConfig:
    ...
    semantic_model: str = "glm-4.6v-flash"
    ...
    def from_env(cls) -> EmailIngestConfig:
        ...
        return cls(
            ...
            chunking_strategy=os.getenv("CHUNKING_STRATEGY", "semantic_consensus"),
            ...
            semantic_model=os.getenv("SEMANTIC_LLM_MODEL", "glm-4.6v-flash"),
            ...
            semantic_json_max_prompt_tokens=int(
            ...
            semantic_consensus_tolerance=int(os.getenv("SEMANTIC_CONSENSUS_TOLERANCE", "20")),
            ...
            semantic_merge_max_embed_chars=int(os.getenv("SEMANTIC_MERGE_MAX_EMBED_CHARS", "2000")),
        )
...
class EmailIngestor:
    ...
    def _build_chunker(self):
        strategy = (self.config.chunking_strategy or "semantic_consensus").lower()
        ...
        if self.llm_provider is None and strategy in {"semantic", "hybrid", "semantic_consensus"}:
            raise ValueError(
                "llm_provider is required for semantic, semantic_consensus, or hybrid chunking"
            )
        ...
        cfg = ChunkingStrategyConfig(
            ...
            semantic_json_max_prompt_tokens=self.config.semantic_json_max_prompt_tokens,
            ...
            semantic_merge_similarity_threshold=getattr(
                self.config, "semantic_merge_similarity_threshold", 0.85
            ),
            semantic_consensus_tolerance=getattr(self.config, "semantic_consensus_tolerance", 20),
            ...
            semantic_merge_max_embed_chars=getattr(
                self.config, "semantic_merge_max_embed_chars", 2000
            ),
        )
